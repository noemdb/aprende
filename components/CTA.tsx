export default function CTA() {
  return (
    <section id="cta" className="py-24 gradient-bg">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <div className="text-white">
          <h2 className="text-4xl sm:text-5xl font-bold mb-6">
            Comienza tu Viaje de Aprendizaje Gamificado
          </h2>
          <p className="text-xl text-indigo-100 mb-10 max-w-2xl mx-auto">
            Únete hoy mismo a StudentQwen y descubre cómo el aprendizaje puede ser motivador, efectivo y divertido.
          </p>

          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <a
              href="https://github.com/noemdb/aprende"
              className="inline-flex items-center justify-center px-8 py-4 bg-white text-indigo-600 font-bold rounded-lg shadow-lg hover:bg-indigo-50 transition-colors"
            >
              <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.65.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
              </svg>
              Descargar desde GitHub
            </a>
            <a
              href="mailto:noemdb@github.com"
              className="inline-flex items-center justify-center px-8 py-4 border-2 border-white text-white font-bold rounded-lg hover:bg-white/10 transition-colors"
            >
              <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              Contactar para Implementación
            </a>
          </div>

          <div className="mt-12 pt-8 border-t border-indigo-400/30">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
              <div>
                <div className="text-3xl font-bold">95%</div>
                <div className="text-indigo-100 mt-2">Tasa de satisfacción</div>
              </div>
              <div>
                <div className="text-3xl font-bold">5K+</div>
                <div className="text-indigo-100 mt-2">Estudiantes activos</div>
              </div>
              <div>
                <div className="text-3xl font-bold">65%</div>
                <div className="text-indigo-100 mt-2">Docentes comprometidos</div>
              </div>
              <div>
                <div className="text-3xl font-bold">20%</div>
                <div className="text-indigo-100 mt-2">Mejora en rendimiento</div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  )
}